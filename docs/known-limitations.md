...
- Native Fabric tags and sensitivity labels are different controls. Tags are defined in Fabric for classification and discovery. When a customer opts into sensitivity-label checks, the CLI verifies presence only; it cannot prove label appropriateness, publication policy, protection, inheritance, DLP, export behavior, or licensing.
...
- The demo does not create real source connections, mirrored databases, shortcuts, semantic models, reports, organizational sensitivity-label policies, DLP policies, audit pipelines, or production monitoring.
